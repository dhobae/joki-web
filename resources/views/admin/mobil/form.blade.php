<div class="form-group">
    ...
    <input type="text" name="plat_mobil" class="form-control" value="{{ old('plat_mobil', $mobil->plat_mobil ?? '') }}"
        required>
</div>
    ...
    <select name="id_merk" class="form-control" required>
        ...
        @foreach ($merks as $merk)
            <option value="{{ $merk->id }}"
                {{ old('id_merk', $mobil->id_merk ?? '') == $merk->id ? 'selected' : '' }}>
                {{ $merk->nama_merk }}
            </option>
        @endforeach
    </select>
...
<div class="form-group">
    ...
    <select name="id_jenis" class="form-control" required>
        ...
        @foreach ($jenis as $j)
            <option value="{{ $j->id }}"
                {{ old('id_jenis', $mobil->id_jenis ?? '') == $j->id ? 'selected' : '' }}>
                {{ $j->nama_jenis }}
            </option>
        @endforeach
    </select>
...
<div class="form-group">
    ...
    <select name="status_mobil" class="form-control" required>
        <option value="Tersedia" {{ old('status_mobil', $mobil->status_mobil ?? '') == 'Tersedia' ? 'selected' : '' }}>
            Tersedia</option>
        <option value="Tidak Tersedia"
            {{ old('status_mobil', $mobil->status_mobil ?? '') == 'Tidak Tersedia' ? 'selected' : '' }}>Tidak Tersedia
        </option>
    </select>
</div>
